...
with tab1:
    ...
    with col2:
        st.markdown(f"""
            <div class="metric-container">
                <div class="metric-value" style="color: {'#32D74B' if time_saved >= 13 else '#FF453A'}">{time_saved}h</div>
    ...
    # --- 🙏 SPIRITUALITÉ (2 cartes séparées) ---
    ...
    if st.button("💾 ENREGISTRER MA JOURNÉE", type="primary", use_container_width=True):
        ...
        toggles = [
            ...
            toggle_hygiene,
            toggle_budget,
            phone_hours <= 3.0  # Bonus si objectif phone atteint
        ]
        ...
        if not df.empty and today_str in df["Date"].values:
            # UPDATE: Remplacer la ligne existante
            df.loc[df["Date"] == today_str, list(new_row.keys())] = list(new_row.values())
            action = "mise à jour"
        else:
            # INSERT: Ajouter nouvelle ligne
        ...
        if save_data(repo, df, contents):
            ...
            st.error("❌ Erreur lors de la sauvegarde. Vérifie ta connexion GitHub.")
...
with tab2:
    ...
    if df.empty:
        ...
        with col2:
            ...
            st.markdown(f"""
                <div class="metric-container">
                    <div class="metric-value" style="color: {color}">{avg_phone:.1f}h</div>
        ...
        with col3:
            ...
            color = "#32D74B" if total_pnl >= 0 else "#FF453A"
            st.markdown(f"""
                <div class="metric-container">
                    <div class="metric-value" style="color: {color}">{total_pnl:.0f}€</div>
        ...
        if not df_weight.empty:
            ...
            st.info("📊 Pèse-toi un vendredi pour voir ton évolution ici")
